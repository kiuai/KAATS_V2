...
async def list_systems(request: Request, db: AsyncSession = Depends(get_db)) -> list[SystemRead]:
    company_id = get_current_company_id(request)
    return await SystemService(db).list_systems(company_id)
...
async def create_system(
    ...
    company_id = get_current_company_id(request)
    return await SystemService(db).create_system(body, company_id=company_id, created_by=user_id)
